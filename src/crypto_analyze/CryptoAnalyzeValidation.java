...
public class CryptoAnalyzeValidation {
    ...
    private static Path fileInputValidation() {
        ...
        while (true) {
            ...
            if (pathFile.equalsIgnoreCase("0")) {                            // Finish the program if user entered 4
    ...
    static void actionWithFileInput() {
        ...
        while (true) {
            ...
            if (input.equalsIgnoreCase("0")) {                                // Finish the program if user entered exit
                System.out.println(TextsForValidation.PROGRAM_IS_FINISHED_OUTPUT);
                System.exit(0);
            ...
            if (input.equals("1")) {
                ACTION_WITH_FILE = "Encrypt";                                     // If user entered 1 set to var ACTION_WITH_FILE
                break;
            } else if (input.equals("2")) {
                ACTION_WITH_FILE = "Decrypt";                                     // If user entered 0 set to var ACTION_WITH_FILE
                break;
            } else if (input.equals("3")) {
                ACTION_WITH_FILE = "Hack";                                        // If user entered 2 set to var ACTION_WITH_FILE
                break;
            } else {
    ...
    static void languageInput() {
        ...
        while(true) {
            ...
            if (input.equalsIgnoreCase("0")) {                               // Finish the program if user entered exit
                ...
                System.out.println(TextsForValidation.EMPTY_INPUT);
            // Verify if scanner is empty
                continue;
            }

            if (input.equals("1")) {
                FILE_TEXT_LANGUAGE = "Eng";
                break;
            } else if (input.equals("2")) {
                FILE_TEXT_LANGUAGE = "Rus";
                break;
            } else {
                System.out.println(TextsForValidation.INVALID_SYMBOL_INPUT);
            }
